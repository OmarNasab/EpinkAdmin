...
function isOnline($userid){
    ...
    if($result->num_rows > 0){
		...
		if($row["last_online"] == null){
			...
			$date_a = new DateTime($row["last_online"]);
			$date_b = new DateTime($currentdatetime);
        ...
        return "User does not exist";                               
										...
										$sssid = 1;
                                          ...
                                          while($row = $result->fetch_assoc()) {
											...
											$date_b = new DateTime($currentdatetime);
											$diff = $date_a->diff($date_b);
											$onlinestatus = isOnline($row["id"]);
											$lso = $diff->y . " years, " . $diff->m." months, ".$diff->d." days ".$diff->h." hour ".$diff->i." minutes";
														  ...
														  <td>'.$sssid.'</td>
                                                            ...
                                                            <td>'. $row["phonenumber"].'</td>
															 <td>'. $row["availability"].'</td>
															  <td>RM'. $row["rider_credit"].'</td>
                                                            <td>'.$onlinestatus.'<br>'. $lso.'</td>
                                                            <td><center><a class="btn btn-primary" href="'.$domain.'/riders/'.$row["id"].'/">Manage</a></center></td>
                                                          </tr>
                                        ';	
										$sssid++;
                                          }
