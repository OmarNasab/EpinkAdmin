...
if(isset($_POST["update_parcel"])){
	...
	$multi_stop_van = cleanInput($_POST["multi_stop_van"]);
	$sql1 = "UPDATE settings SET setting_value='$parcel_first_km' WHERE setting_item='parcel_first_km_van'";
	$sql2 = "UPDATE settings SET setting_value='$parcel_first_km_price' WHERE setting_item='parcel_first_km_price_van'";
	$sql3 = "UPDATE settings SET setting_value='$parcel_second_km' WHERE setting_item='parcel_second_km_van'";
	$sql4 = "UPDATE settings SET setting_value='$parcel_second_km_price' WHERE setting_item='parcel_second_km_price_van'";
	$sql6 = "UPDATE settings SET setting_value='$parcel_third_km_price' WHERE setting_item='parcel_third_km_price_van'";
	$sql6 = "UPDATE settings SET setting_value='$multi_stop_van' WHERE setting_item='multi_stop_van'";
...
$sqlpriceperkm = "SELECT * FROM settings WHERE setting_item='parcel_first_km_van'";
...
$sqlpriceperkm = "SELECT * FROM settings WHERE setting_item='parcel_first_km_price_van'";
$priceperkmresult = $db->query($sqlpriceperkm);
if ($priceperkmresult->num_rows > 0) {
	$pricerow = $priceperkmresult->fetch_assoc();
	$parcel_first_km_price = $pricerow["setting_value"];
}
$sqlpriceperkm = "SELECT * FROM settings WHERE setting_item='parcel_second_km_van'";
$priceperkmresult = $db->query($sqlpriceperkm);
if ($priceperkmresult->num_rows > 0) {
	$pricerow = $priceperkmresult->fetch_assoc();
	$parcel_second_km = $pricerow["setting_value"];
}
$sqlpriceperkm = "SELECT * FROM settings WHERE setting_item='parcel_second_km_price_van'";
$priceperkmresult = $db->query($sqlpriceperkm);
if ($priceperkmresult->num_rows > 0) {
	$pricerow = $priceperkmresult->fetch_assoc();
	$parcel_second_km_price = $pricerow["setting_value"];
}
$sqlpriceperkm = "SELECT * FROM settings WHERE setting_item='parcel_third_km_van'";
$priceperkmresult = $db->query($sqlpriceperkm);
if ($priceperkmresult->num_rows > 0) {
	$pricerow = $priceperkmresult->fetch_assoc();
	$parcel_third_km = $pricerow["setting_value"];
}
$sqlpriceperkm = "SELECT * FROM settings WHERE setting_item='parcel_third_km_price_van'";
$priceperkmresult = $db->query($sqlpriceperkm);
if ($priceperkmresult->num_rows > 0) {
	$pricerow = $priceperkmresult->fetch_assoc();
	$parcel_third_km_price = $pricerow["setting_value"];
}
$sqlpriceperkm = "SELECT * FROM settings WHERE setting_item='multi_stop_van'";
$priceperkmresult = $db->query($sqlpriceperkm);
if ($priceperkmresult->num_rows > 0) {
	$pricerow = $priceperkmresult->fetch_assoc();
	$multi_stop_van = $pricerow["setting_value"];
}
?>
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Van Parcel Delivery Setting</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="container-fluid">
			<form method="POST">
				...
				<div class="form-group">
					<label for="">Van Multi Delivery Stop Charges</label>
					<input type="number" name="multi_stop_van" id="multi_stop_van" class="form-control" step=".01" value="<?php echo $multi_stop_van; ?>">
				</div>	
				<div class="form-group">
