...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '
            <tr>
                <td>
					<p class="text-xs">'.$requestdate.'</p>
					<p class="text-xs">Patient: '.$row["patientname"].'</p>
					<p class="text-xs">Requester: '.$row["fullname"].'</p>
				</td>
                <td class="">' . $row["request_status"] . '</td>
                <td><p class="strong">' . $carename . '</p><p class="text-xs">Problem: ' . $problem . '</p><p class="text-xs">Type: '.$caretype.'</p><p class="text-xs">Category: '.$carecategory.'</p></td>
				<td>' . $row["careprice"] . '</td>                    
                <td><a class="btn btn-primary" href="https://epink.health/admin/ecare-job-manager/update/' . $row["id"] . '/">Update</a> <a class="btn btn-primary" href="' . $domain . '/ecare-job-manager/delete/' . $row["id"] . '/">Delete</a></td>
            </tr>
			';
    }
} else {
