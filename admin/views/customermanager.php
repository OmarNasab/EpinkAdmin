...
if (isset($_POST["submitupdateusers"]) && $_POST["csrf"] == $_SESSION["csrftoken"])
{
	...
	$ethnicity = cleanInput($_POST["editethnicity"]);
	$nationality = cleanInput($_POST["editindustry"]);
	$industry = cleanInput($_POST["editindustry"]);
	$specification = cleanInput($_POST["editspecification"]);
	$editapproved = cleanInput($_POST["editapproved"]);
     
    $sql = "UPDATE users SET email='$email', phonenumber='$phonenumber', wallet='$editwallet', customer_approved='$editapproved', ethnicity='$ethnicity', nationality='$nationality', industry='$industry', specification='$specification' WHERE  id='$id'";
    if ($db->query($sql) === true)
    {
        $row["card"] = "green";
        $row["status"] = "Successfull";
                    ...
                    <div class="card" style="width: 100%;">
                        <?php
                            if($usersobject["profile_img"] == 'img/default_profile_picture.jpg'){
                            	$usersobject["profile_img"] = 'https://placehold.it/500x500';
                            }
                            ?>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form id="editusers" method="POST" enctype="multipart/form-data" action="<?php
						...
						<div class="form-group">
                            <label for="editemail">Ethnicity</label>
                            <input type="text" id="editethnicity" class="form-control" name="editethnicity" value="<?php
                                echo $usersobject["ethnicity"];
                                ?>">
                        </div>
						<div class="form-group">
                            <label for="editemail">Industry</label>
                            <input type="text" id="editindustry" class="form-control" name="editindustry" value="<?php
                                echo $usersobject["industry"];
                                ?>">
                        </div>
						<div class="form-group">
                            <label for="editemail">Nationality</label>
                            <input type="text" id="editnationality" class="form-control" name="editnationality" value="<?php
                                echo $usersobject["nationality"];
                                ?>">
                        </div>
						<div class="form-group">
                            <label for="editspecification">Population Category</label>
                            <input type="text" id="editspecification" class="form-control" name="editspecification" value="<?php echo $usersobject["specification"]; ?>">
                        </div>
                        <div class="form-group">
                            <label for="editemail">Approved</label>
							<select id="editapproved" class="form-control" name="editapproved">
								<?php
								if($usersobject["customer_approved"] == "true"){
									echo '
